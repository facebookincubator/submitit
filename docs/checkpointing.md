...
## The basics of checkpointing with `submitthem`
...
In practice, when requeuing, `submitthem` will check if the callable has a `__submitthem_checkpoint__` or `checkpoint` method.
...
```python
def checkpoint(self, *args: Any, **kwargs: Any) -> submitthem.helpers.DelayedSubmission:
    return submitthem.helpers.DelayedSubmission(self, *args, **kwargs)  # submits to requeuing
```
If this kind of checkpoint is sufficient for you, you can derive your callable from `submitthem.helpers.Checkpointable` which implements this very function.
...
For a basic example of a checkpointable callable, checkout the code from `submitthem.helpers.FunctionSequence` in [helpers.py](../submitthem/helpers.py).


## Example - Training and checkpointing a model

The following example provides a recipe for checkpointing the training of a model. It is more complex because we do not want to rely on `submitthem` to pickle the model. This recipe has not been fool proofed yet, I am happy to help if you encounter any issue ;)
...
class NetworkTraining:
    ...
    def checkpoint(self, checkpointpath: str) -> submitthem.helpers.DelayedSubmission:
        ...
        return submitthem.helpers.DelayedSubmission(training_callable, checkpointpath)
```
...
executor = submitthem.AutoExecutor(folder="logs_training", slurm_max_num_timeout=3)
