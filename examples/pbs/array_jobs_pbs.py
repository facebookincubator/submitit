...
def main():
    ...
    print("Submitting 50 dataset processing jobs...")
    jobs = []
    for dataset_id in range(50):
    ...
    for job in jobs:
        try:
            ...
            if result["success"]:
                print(f"Job {job.job_id}: processed {result['total_processed']} items")
            else:
                print(f"Job {job.job_id}: completed with {result['errors']} errors")
